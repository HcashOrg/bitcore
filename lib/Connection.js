...
function Connection(socket, peer, opts) {
  ...
  this.network = networks[this.config.network] || networks.hcdlivenet;
  this.socket = socket;
  this.peer = peer;
...
Connection.prototype.sendMessage = function(command, payload) {
  try {
    ...
    var checksum = new Buffer(crypto.createHash('blake256').update(payload).digest('binary'), 'binary').slice(0,4);
...
Connection.prototype.processData = function() {
  ...
  var checksum = this.buffers.slice(20, 24);
  ...
  if (checksum !== null) {
    var checksumConfirm = new Buffer(crypto.createHash('blake256').update(payload).digest('binary'), 'binary').slice(0,4);
...
Connection.prototype.parseMessage = function(command, payload) {
  ...
  switch (command) {
    ...
    case 'block':
      ...
      data.stake_root = block.stake_root;
      data.votebits = block.votebits;
      data.finalstate = block.finalstate;
      data.voters = block.voters;
      data.freshstake = block.freshstake;
      data.revocations = block.revocations;
      data.poolsize = block.poolsize;
      data.bits = block.bits;
      data.sbits = block.sbits;
